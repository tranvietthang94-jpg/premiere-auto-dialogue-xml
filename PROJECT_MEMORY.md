# Memory handoff — Premiere Auto Dialogue XML, Phase 00–10 + Phase 12 đang thực hiện
...
Phase 12 đã mở ngày 2026-08-14 từ clean `main` commit `3f41d7cd2d8b4d90241b4bd2f50f798e23d65796` trên branch `codex/phase12-premiere-input-compatibility`. Cổng đầu chỉ mở sequence nguyên `24/25/30 fps NDF`; source mono PCM `48 kHz`, master stereo, routing/gain/Enabled/M19 vẫn giữ Phase 10. Stereo source, sample rate/routing mới, `23,976/29,97` và drop-frame chưa mở. Slice 12A thêm `PremiereNdfFrameGrid` và parser fixture; 12B truyền grid qua toàn bộ analysis; 12C nối cùng grid qua DOM/streaming XML, marker/review, output validator và PCM validator. Audit production tăng `1.6` → `1.8`, ghi timing policy `phase12-integer-ndf-exact-frame-grid-v1`; PCM evidence `1.2`; schema `1.8+` thiếu/sai timing bị từ chối, audit cũ chỉ fallback 25 fps. Inspector nay nhận 24/30 khi sequence và từng clip ghi rõ `ntsc=FALSE`. End-to-end synthetic package đạt ở 24/25/30; Release `198/198`, build 0 warning/error, format/diff sạch. Đây chưa phải Premiere adoption; bước tiếp theo là Slice 12D regression HGE2/full HGE 25 fps, M19, publish/installer.
