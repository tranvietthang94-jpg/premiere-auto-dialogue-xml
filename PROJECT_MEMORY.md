...
Stable closeout mở ngày 2026-08-26 trên `codex/v0.1.0-stable`, PR `#23`. Không đổi code audio/XML hoặc UI. Rà soát pre-release từ `fc6ead9` đạt Release build `0 warning/0 error`, `223/223` test, format/diff sạch, policy Premiere 24/30 `8/8`, policy multi-boundary, publish self-contained `411` file, dependency scan sạch và GitHub artifact API `0`; PR CI đầu `32925672098` đạt toàn bộ workflow trong `3 phút 42 giây`. Release phải có CI cuối xanh, tag annotated `v0.1.0` trên post-merge main và gói installer ký nội bộ local với `privateKeyIncluded=false`; GitHub không nhận binary hoặc private artifact.
