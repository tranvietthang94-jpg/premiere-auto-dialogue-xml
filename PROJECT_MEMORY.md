# Memory handoff — Premiere Auto Dialogue XML, Phase 00–16

Ngày cập nhật: 2026-08-24 (Asia/Saigon). Đây là memory source-of-truth để một phiên Codex mới tiếp tục nâng cấp dự án mà không làm lại các phase đã hoàn tất. Phase 11 là research-only và không merge; Phase 14–15 giữ candidate tooling nhưng production writer không adopt transition; Phase 16 đang chờ Premiere gate.
...
- Phase 16 đang chạy trên draft PR `#22`: source-peak safety gate loại boundary khi bỏ một frame quanh transition làm expected peak lệch quá `0,1 dB`, không tự bù gain. HGE2 gate loại đúng A2/frame 11140, chọn candidate 12 transition mới và comparator với generated XML có semantic mismatch `0`; local `223/223` test đạt. Chưa production-adopt; chờ Premiere XML/PCM.
...
- RC1 ký nội bộ lịch sử vẫn được giữ cục bộ trong `private-artifacts/release-v0.1.0-rc1`; draft release GitHub đã được xóa theo chính sách source-only. Không đổi RC1 hoặc phát hành bản mới.
...
Phase 16 mở ngày 2026-08-24 từ `main` commit `d3d0d4bf56efcca8f144fee800d9528c6ddb203d` trên `codex/phase16-conservative-transition-adoption`, draft PR `#22`. `PremiereTransitionGainSafetyGate` đọc source PCM phrase, bỏ guard một frame mỗi phía Enabled và chỉ cho qua khi retained expected peak lệch không quá `0,1 dB`; thiếu phrase/gain/source hoặc source peak không khớp audit đều fail-closed. `PremiereConstantGainSafeBatchPlanner` thêm one-transition-per-phrase conflict; CLI mới chỉ tạo candidate, không nối production writer. HGE2 scan có 1.282 transient, capture 785: 633 Eligible, 150 thiếu phrase provenance và 2 expected-peak-loss bị loại. A2/frame 11140 bị loại với retained delta `-10,1472188 dB`; A3/frame 27268 bị loại `-3,5084822 dB`. Candidate chọn 12 transition (5 E→D, 6 D→E, 1 gain), XML SHA-256 `2F3FBD0DEEEFE3AA51A30A30141631A39435DE8378F38650DD646DA203DBAB54`; comparator với generated Phase 13 mismatch 0 trên 8.291 clip, 4.488 Enabled, 3.803 Disabled, 5.207 marker và gain. Local build sạch, `223/223` test, Phase 13 policy 8/8, Phase 15 normalization policy và format đạt. Bước còn lại là import đúng candidate trong `private-artifacts/phase16-conservative-transition-hge2-20260824-1`, re-export XML và WAV mono full-sequence A2/A3/A5/A6/A7; chưa production-adopt.
