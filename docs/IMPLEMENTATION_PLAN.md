...
12. `codex/phase11-calibrated-multimic-bleed`
...
Phase 11 trên branch `codex/phase11-calibrated-multimic-bleed` đã hoàn tất nghiên cứu shadow, candidate `d150f5a`, CI `31720635957` xanh. Slice 11A khóa corpus; 11B thêm calibrator có hướng; 11C nối scorer `120–250 ms`, minimum `2` cửa sổ/`75%`, project shadow và audit `1.7`; 11E qua HGE2 HEAD `73,658 giây / 299,4 MB` và full HGE `72 phút 31,005 giây / 1.435,1 MB`. Cả hai corpus có 42/42 pair thiếu support, mọi candidate `NoCalibration`, production changed/coverage mismatch/lost Enabled đều `0`; review CSV giữ nguyên hash Phase 10. Release đạt `171/171`; build/publish/installer CI đạt. Review ngày 2026-08-14 không adopt/merge PR #16 vì chưa đo được lợi ích chất lượng, trong khi audit tăng `33,6–34,0%` và peak RAM full HGE tăng `114,7 MB`. Branch/evidence được giữ làm nghiên cứu; `main` và RC1 không đổi. Không mở adoption hoặc tăng production Disabled khi chưa có nhãn Target hợp lệ. Xem [PHASE11_CALIBRATED_MULTIMIC_BLEED.md](PHASE11_CALIBRATED_MULTIMIC_BLEED.md).
