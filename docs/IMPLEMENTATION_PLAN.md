...
Phase 11 đang thực hiện trên branch `codex/phase11-calibrated-multimic-bleed`. Slice 11A khóa corpus PCM tổng hợp 11 kịch bản; Slice 11B thêm calibrator có hướng, minimum 3 anchor, outlier/leave-one-region-out và evidence bounded; Slice 11C nối scorer `120–250 ms`, minimum `2` cửa sổ/`75%`, project shadow và audit `1.7`. Validator giữ mọi Speech/Ambiguous Enabled và chặn shadow sai policy/hash/provenance trước publication. Release đạt `171/171`; regression writer xác nhận XML hash không đổi khi chỉ thêm shadow. Bước tiếp theo là pilot shadow-only HGE2/full HGE; không mở adoption hoặc tăng production Disabled khi chưa có nhãn Target hợp lệ. Xem [PHASE11_CALIBRATED_MULTIMIC_BLEED.md](PHASE11_CALIBRATED_MULTIMIC_BLEED.md).
