...
17. `codex/phase16-conservative-transition-adoption` — source-peak gain safety gate bảo thủ trước production adoption
...
Phase 13 đã đạt; implementation merge PR `#18` tại `ad89d2a`, PR CI `32562437223` và hậu merge CI `32562657372` đạt mà không đổi VAD/noise/bleed/gain/routing/marker/XML semantic. CI có `8/8` policy case khóa strict/explicit sequence-depth cho cả 24/30. Paired scan đọc PCM một lần cho hai detector độc lập; audit `1.9` giữ full count/SHA-256 và mẫu bounded. HGE2 giảm RAM/audit với runtime trong `+5%`; full HGE đạt `75:52,4`/`1.218,3 MB`/audit `157,52 MB`, coverage mismatch/lost Enabled/newly Enabled `0`, M19 Enabled. Theo quyết định sản phẩm ngày 2026-08-23, GitHub chỉ lưu source code: CI build/smoke-test installer tạm thời nhưng không upload; installer dùng thật chỉ được tạo và giữ cục bộ. Source-only PR `#19` CI `32645714062` đạt toàn bộ gate. Xem [PHASE13_RELIABILITY_PERFORMANCE.md](PHASE13_RELIABILITY_PERFORMANCE.md).
...
Phase 16 đang mở gain-safety gate bảo thủ cho transition candidate. Policy đọc source PCM thật của phrase, bỏ một frame quanh boundary và chỉ giữ transition khi peak còn lại vẫn bảo toàn expected post-routing peak trong `0,1 dB`; thiếu provenance hoặc nhiều boundary tác động cùng phrase đều bị loại. Production writer/app mặc định chưa đổi cho tới khi HGE2 candidate qua Premiere XML/PCM mới. Xem [PHASE16_CONSERVATIVE_TRANSITION_ADOPTION.md](PHASE16_CONSERVATIVE_TRANSITION_ADOPTION.md).
